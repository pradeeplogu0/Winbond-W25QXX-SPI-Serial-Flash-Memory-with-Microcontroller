...
void setup() {
  ...
  Serial.print(F("Manufacturer ID        :"));Serial.println(manufacturer_id, HEX);
  Serial.print(F("device ID              :"));Serial.println(device_id, HEX);
  Serial.println(F("Total Memory(KB)   ` :"));
  Serial.println(F("Availed Memory(%)    :"));

  read_block(0x000000, 16, 20, true);             // read data  
//download_rom(true);                             // download entier rom into a file
...
void get_device_info(byte &manufacturer_id, byte &device_id) {
  ...
  spi_transfer(0x00);
  spi_transfer(0x00);
  manufacturer_id = spi_transfer(INFO); //get data byte
  device_id = spi_transfer(INFO); //get data byte
